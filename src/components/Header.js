...
const Header = () => {
  ...
  return (
    <div
      className="flex justify-between  bg-slate-600 border-slate-700  shadow-xl
         py-2"
    >
      <div className="w-24 mx-2">
        <img
          className="image border-black shadow-lg rounded-md"
          src={LogoImage}
        />
      </div>
        ...
        <ul className="flex">
          ...
          <li className="px-4 text-white relative">
            <Link to="/cart" className="inline-block">
              ...
              <span className="absolute top-0 right-0 bg-slate-600 text-white rounded-full px-1 py-1">
                {cartItems.length}
              </span>{" "}
              {/* Cart Count */}
            </Link>
